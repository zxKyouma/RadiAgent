#!/usr/bin/env python3
"""Diagnose whether end-to-end failures are due to localization, proposal quality, or selection."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--oracle-success-dice-threshold', type=float, default=0.5)
...
def classify_failure(localizer_hit: bool, selected_dice: float | None, oracle_best_dice: float | None, threshold: float) -> str:
    selected_ok = selected_dice is not None and selected_dice >= threshold
    oracle_ok = oracle_best_dice is not None and oracle_best_dice >= threshold
    if selected_ok:
        return 'success'
    if not localizer_hit:
        return 'localizer_miss'
    if oracle_ok:
...
def _json_safe(value):
    ...
    if isinstance(value, np.ndarray):
        return value.tolist()
    if hasattr(value, 'dice') and hasattr(value, 'iou') and hasattr(value, 'pred_pixels') and hasattr(value, 'gt_pixels'):
        return {
            'pred_pixels': int(value.pred_pixels),
            'gt_pixels': int(value.gt_pixels),
            'intersection': int(value.intersection),
            'dice': float(value.dice),
            'iou': float(value.iou),
        }
...
def maybe_summary(candidate):
    return None if candidate is None else _json_safe(candidate_to_summary(candidate))
...
def main() -> int:
    ...
    for case_id in cases:
        ...
        localizer_hit = any(set(h.slice_indices) & lesion_slice_set for h in artifacts.localizer_hypotheses)
        ...
        best_text = max(
            (c for c in artifacts.candidates if c.metadata.get('generator') == 'text' and c.refined_metrics is not None),
            key=lambda c: float(c.refined_metrics.dice),
            default=None,
        )
        best_visual = max(
            (c for c in artifacts.candidates if c.metadata.get('generator') == 'visual' and c.refined_metrics is not None),
            key=lambda c: float(c.refined_metrics.dice),
            default=None,
        )

        selected_dice = None if selected is None or selected.refined_metrics is None else float(selected.refined_metrics.dice)
        oracle_best_dice = None if oracle_best is None else float(oracle_best.refined_metrics.dice)
        failure_mode = classify_failure(localizer_hit, selected_dice, oracle_best_dice, args.oracle_success_dice_threshold)
        ...
        case_results.append({
            ...
            'selection_strategy': None if artifacts.selection is None else artifacts.selection.strategy,
            'failure_mode': failure_mode,
            'selected': maybe_summary(selected),
            'oracle_best': maybe_summary(oracle_best),
            'best_text_by_dice': maybe_summary(best_text),
            'best_visual_by_dice': maybe_summary(best_visual),
            'candidate_count': len(artifacts.candidates),
            'text_candidate_count': sum(1 for c in artifacts.candidates if c.metadata.get('generator') == 'text'),
            'visual_candidate_count': sum(1 for c in artifacts.candidates if c.metadata.get('generator') == 'visual'),
        })

    summary = {
        'localizer': args.localizer,
        'oracle_success_dice_threshold': args.oracle_success_dice_threshold,
        'case_count': len(case_results),
        'failure_counts': failure_counts,
        'avg_elapsed_sec': sum(item['elapsed_sec'] for item in case_results) / max(len(case_results), 1),
        'total_elapsed_sec': time.perf_counter() - total_start,
        'results': case_results,
    }

    output_path = Path(args.output_json) if args.output_json else RESULTS_DIR / f'brain_mri_oracle_analysis_{args.localizer}.json'
